...
def parse_args():
    ...
    model_group.add_argument(
        "--is-audio", action="store_true", help="Whether the target model is a model with audio input"
    )
    model_group.add_argument("--audio-root", type=str, default=None)
...
def log_verification_samples(output_data, tokenizer, print_fn):
    """
    辅助函数：解码并打印输入与模型预测的 Token
    """
    # 1. 解码 Input Text
    ...
    predicted_text = tokenizer.decode(predicted_ids, skip_special_tokens=False)

    # 3. 格式化打印
    sep = "-" * 30
    print_fn(f"\n{sep} DEBUG: Eagle3 Verification {sep}")
    print_fn(f"--- [INPUT TEXT] ---\n{input_text}")
    print_fn(f"--- [PREDICTED TEXT] ---\n{predicted_text}")
    print_fn(f"{sep * 3}\n")
...
class HiddenStatesGenerator:
    ...
    def generate(
        ...
        compare_output_data: bool = False
    ):
        """
        ...
        for batch_idx, batch in enumerate(progress_bar):
            ...
            filtered_batch = {
                ...
                "input_features": batch["input_features"][valid_indices_in_batch],
                "feature_attention_mask": batch["feature_attention_mask"][valid_indices_in_batch]
            }
            del batch
            if num_valid == 0:
            ...
            if compare_output_data:
                with torch.no_grad():
                    output_data = self.model.generate_eagle3_data(**filtered_batch_gpu)
                    log_verification_samples(output_data, tokenizer, print_with_rank)
