...
{%- for message in messages %}
    ...
    {%- else %}
        {%- set msg_content = namespace(text="") %}
        {%- for mcontent in message.content %}
            {%- if mcontent.type == 'image' or 'image' in mcontent or 'image_url' in mcontent %}
                {%- set msg_content.text = msg_content.text~"<|vision_start|><|image_pad|><|vision_end|>" %}
            {%- elif mcontent.type == 'audio' or 'audio' in mcontent or 'audio_url' in mcontent %}
                {%- set msg_content.text = msg_content.text~"<|audio_start|><|audio_pad|><|audio_end|>" %}
            {%- elif mcontent.type == 'video' or 'video' in mcontent %}
                {%- set msg_content.text = msg_content.text~"<|vision_start|><|video_pad|><|vision_end|>" %}
            {%- elif mcontent.type == 'text' %}
                {%- set msg_content.text = msg_content.text~mcontent.text %}
            {%- endif %}
        {%- endfor %}
        {%- set content = msg_content.text %}
    {%- endif %}
